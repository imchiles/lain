...
public class CrcCheck {
	...
	public static int calcCrc16(byte[] data, int offset, int len) {
		return calcCrc16(data, offset, len - 2, 0xffff);
	}
	...
	public static boolean CrcEqualsResult( byte [] bytes){
		int crc = calcCrc16(bytes);// crc检验
		String crcc = String.format("%04x", crc).toUpperCase(); // crc返回值处理
		String bys=HumitureTypeConvert.byteArrayToString(bytes);
		String crc1= bys.substring(bys.length()-4);
		String crc2=crcc.substring(2)+crcc.substring(0, 2);
		return crc2.equals(crc1);
	}
}
